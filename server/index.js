...
// 基于FORM-DATA上传数据
app.post('/single_upload', async (req, res) => {
    let {
        files
...
// 上传BASE64
app.post('/single_upload_base64', (req, res) => {
    let {
        chunk,
        filename
    } = req.body;
...
// 切片上传
app.post('/single_upload_chunk', async (req, res) => {
    let {
        fields,
        files
...
app.get('/upload_already', async (req, res) => {
    ...
    try {
        ...
        res.send({ code: 0, codeText: '', fileList: fileList });
    } catch (err) {
        res.send({ code: 0, codeText: '', fileList: fileList });
    }
})
...
// 切片合并
app.get('/single_upload_merge', (req, res) => {
    let {
        hash
    } = req.query;
